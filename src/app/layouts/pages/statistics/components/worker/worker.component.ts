...
export class WorkerComponent extends BaseComponent implements OnInit {
  data: ITasks[] = [];
  ...
  updateStatsWithParams(params) {
    ...
    this.getStatisticsWorker();
  }

  getStatisticsWorker() {
    this.data = [];
    this.statisticsService.getStatsWorker(this.queryLine).subscribe((res) => {
      this.data = res;
      this.createChart();
    });
  }
